...
class JourneyService {
  ...
  async getReflectionSummary(userId, period, date) {
    ...
    const positiveMoods = new Set(['calm', 'confident', 'energized']);
    const negativeMoods = new Set(['anxious', 'sad', 'overwhelmed']);
  ...
  _generateWeekNarrative(week, weekMoods, weekIndex, totalWeeks) {
    ...
    const anxiousCount = moodIds.filter(m => m === 'anxious' || m === 'overwhelmed').length;
    const calmCount = moodIds.filter(m => m === 'calm' || m === 'confident').length;
